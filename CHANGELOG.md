...
### Added
- Implemented AST type-aware mutator sampling and discard reducer in `ReturnValueMutator` and `ArithmeticOperatorMutator` to inspect enclosing return types and operand expressions, suppressing statically invalid mutations (e.g. non-numeric return values mutated to 0, non-boolean return values mutated to false/true, and string concatenation mutated with arithmetic `-`/`/`), reducing compilation discard overhead.
...
- Implemented Code Climate Issue JSON (`CodeClimateReportExporter`, `--codeclimate`) report exporter in `kronenberg-cli`.
